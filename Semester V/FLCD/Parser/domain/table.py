...
class Table:
    ...
    def __init__(self, symbols):
        self.actionsForStates = {} # Key: state -> Value: action(Shift, Reduce, Accept)
        self.stateToStateMap = {} # Key: state -> Value: ( Key: symbol -> Value: state )
        self.symbolsToStateDefault = {} # Key: symbol -> Value: state
        for symbol in symbols:
            self.symbolsToStateDefault[symbol] = -1 # -1 means no transition(default)

    def addSymbolToState(self, fromState, symbol, toState):
        if fromState not in self.stateToStateMap.keys():
            symbolsToState = copy.deepcopy(self.symbolsToStateDefault) # copy the default values
            #use deepcopy
            self.stateToStateMap[fromState] = symbolsToState
            if self.stateToStateMap[fromState][symbol] == -1: # if the transition is not defined
